 ...
 * 制造信息解析.
 *
 * \param data 回传制造信息数据结构
 * \return		解析的制造信息数据结构
 */
UI_MAKEINFO_DATA SC::getMakeinfoData(BACK_MAKEINFO_DATA data)
{
    UI_MAKEINFO_DATA info;

    QString sn  = nullptr;
    quint32 dsn = 0;

    // DSN
    memcpy(&dsn, data.DSN, 4);
    dsn = __changeuint32(dsn);
    info.DSN = QString("%1").arg(dsn, 8, 16, QLatin1Char('0'));
    // SN
    sn = QString::fromLocal8Bit((char*)data.SN, sizeof(data.SN));
    info.SN = sn;
    // 制造信息
