...
LightButton::LightButton(QWidget *parent) : QWidget(parent)
{
	...
	connect(&showTimer, &QTimer::timeout, this, &LightButton::onShowTimer);
}

void LightButton::setCanExpand(bool canExpand)
{
	...
	if (canExpand)
	{
		...
		if (mouseEvent->type() == QEvent::MouseButtonPress) {
			if (this->rect().contains(mouseEvent->pos()) && (mouseEvent->button() == Qt::LeftButton)) {
				...
				emit expandStateChanged(isExpanding);
				update();
			}
...
void LightButton::paintEvent(QPaintEvent *)
{
    ...
    if (showRect) {
		...
		if (canExpand) {
			...
			painter.setPen(pen);
			if (!isExpanding) {
				painter.drawLine(0, -len,  0, len );
			}
            painter.drawLine(-len, 0, len , 0);
...
void LightButton::drawText(QPainter *painter)
{
	...
	QFontMetrics fontMtr(font);
	int txtLen = fontMtr.horizontalAdvance(text);
	font.setPixelSize(10000 / txtLen);
    painter->setFont(font);
    painter->setPen(textColor);
...
void LightButton::onShowTimer()
{
	showTimer.stop();
	if (isEnter && !isExpanding)
	{
		emit expandStateChanged(true);
	}
}

void LightButton::hideSlot()
{
	isExpanding = false;
	update();
}
