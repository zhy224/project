...
CN* CN::cn = nullptr;
QTranslator *CN::translator = nullptr;
 ...
 *
 */
void CN::switchLanguage(quint8 index)
{
    ...
    if (index == 0) {
        if(!translator->load(":/Resource/translation/Ch.qm"))
        {
            qDebug()<<"load Chinese.qm error";
            return;
        }

    }
    else {
        if(!translator->load(":/Resource/translation/En.qm"))
        {
            qDebug()<<"load English.qm error";
            return;
        }
    }
...
void CN::readPendingDatagrams()
{
    ...
    while(udpSocket->hasPendingDatagrams())
    {
        ...
        // 命令符
        quint8 fram = arr.at(0);
        switch (fram)
        {
            // 通过判断是否回传数据来判断网络是否连接成功
        case DT::BackHeader:
        {
            if (!this->isNetwork()) {
                // 有数据接收 则网络连接
